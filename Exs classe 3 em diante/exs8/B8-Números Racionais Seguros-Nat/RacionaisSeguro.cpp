...
class RacionalQuotientZeroException : public std:: runtime_error{
    public:
        RacionalQuotientZeroException():runtime_error("Denominador igual a zero.")
        { }
...
void tRacional :: setDenominador(int denominador){
    ...
    if(denominador == 0){
        throw RacionalQuotientZeroException();
    }
    else{
...
int main(){
  ...
  while(std :: cin >> n1 >> d1 >> op >> n2 >> d2){
    ...
    catch(RacionalQuotientZeroException e){
